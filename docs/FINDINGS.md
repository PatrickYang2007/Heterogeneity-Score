...
2. **Sweep context width** — §3 predicts 256–512 bp wins on both accuracy and ~8×
   compute, and buys back the epochs currently lost to overfitting. A `--crop`
   flag that does this without regenerating data lands in a follow-up branch
   (`experiment/data-integrity-and-local-context`); until then, set `WINDOW` and
   re-run `widen_windows.py`.
3. **Give the head the labelled region**, not just the window-wide attention
   average (§3). Also in the follow-up branch, as `--center-pool`.
